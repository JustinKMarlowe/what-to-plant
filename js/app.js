...
const app = {
  ...
  /**
   * Build a fallback URL using Wikimedia's Special:Redirect/file endpoint.
   * This is the officially recommended way to hotlink Wikimedia images.
   */
  _wikiRedirectUrl(imageFile, width) {
    if (!imageFile) return '';
  ...
  _plantCard(plant, index) {
    ...
    // Primary: direct thumb URL with referrerpolicy.
    // Fallback 1: Wikimedia Special:Redirect endpoint (officially recommended for hotlinking).
    // Fallback 2: placeholder SVG.
    const primarySrc = plant.image;
    const fallbackSrc = plant.imageFile
      ? this._wikiRedirectUrl(plant.imageFile, 400)
      : '';
    ...
    const errorHandler = fallbackSrc
      ? `this.onerror=function(){this.onerror=null;this.src='${placeholderSrc}';};this.src='${fallbackSrc}';`
      : `this.onerror=null;this.src='${placeholderSrc}';`;
