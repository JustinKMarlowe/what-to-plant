...
const app = {
  ...
  _renderSoilJumpNav(grouped) {
    const container = document.getElementById('soil-jump-nav');
    if (!container) return;

    const soilKeys = Object.keys(grouped);
    if (soilKeys.length === 0) {
      container.innerHTML = '';
      return;
    }

    const buttons = soilKeys.map(soilKey => {
      const soil = SOIL_TYPES[soilKey];
      const count = grouped[soilKey].length;
      return `<a class="soil-jump-btn" href="#soil-${soilKey}" onclick="event.preventDefault(); document.getElementById('soil-${soilKey}').scrollIntoView({behavior:'smooth'});">
        <span class="soil-jump-icon">${soil.icon}</span>
        <span>${soil.name}</span>
        <span class="soil-jump-count">${count}</span>
      </a>`;
    }).join('');

    container.innerHTML = `
      <p class="soil-jump-nav-label">Jump to soil type:</p>
      <div class="soil-jump-nav-list">${buttons}</div>
    `;
  },
  ...
  _renderPlants(zone, month, filter) {
    ...
    // Render the jump nav
    this._renderSoilJumpNav(grouped);
    ...
    for (const [soilKey, plants] of Object.entries(grouped)) {
      ...
      html += `
        <div class="soil-section" data-soil="${soilKey}" id="soil-${soilKey}">
          <div class="soil-section-header">
            <div class="soil-icon ${soilKey}-soil">${soil.icon}</div>
            <div>
              <h2>${soil.name}</h2>
              <p class="soil-header-desc">${soil.desc}</p>
              <p class="soil-header-count">${plantCount} plant${plantCount !== 1 ? 's' : ''} recommended</p>
            </div>
          </div>
